  ...
  <div class="downloadPdf">
    <el-form class="not-inline" label-width="240px" size="medium">
      ...
      <el-form-item label="体测日期：" required>
        <el-select v-model="date" clearable placeholder="请选择" :disabled="!companyId">
    ...
    <download v-if="1 || dataIndex > -1 && downloading" :obj="classData[dataIndex]" @over="getNowData()" />
  ...
  methods: {
    ...
    getClassData () {
      ...
      this.$api.post('/physical-report/class/report/pdf', {
        data: {
          schoolId: 1 || this.userInfo.schoolId,
          date: '2019-11-14'
        }
      }).then(res => {
        if (res.code === '00000') {
